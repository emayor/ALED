...
public class Laberinto {
    ...
    private void resolver(int x, int y) {
    	...
    	// Si ya has estado o has encontrado el centro, termina.
    	if(encontrado || visitado[x][y]){
       		return;
    	}
    	
    	// Marca que ha visitado la posición.
    	visitado[x][y] = true;
    	...
    	//Marca que has llegado al centro
    	if(x == Math.round(N/2.0) && y == Math.round(N/2.0)){
    		encontrado = true;
    	}
    	
    	//Si en el norte no hay pared, resuleve.
    	if(norte[x][y] == false){
    		resolver(x, y+1);
    	}
    	
    	//Si en el este no hay pared, resuelve.
    	if(este[x][y] == false){
    		resolver(x+1, y);
    	}
    	
    	//Si en el sur no hay pared, resuelve.
    	if(sur[x][y] == false){
    		resolver(x, y-1);
    	}
    	
    	//Si en el oeste no hay pared, resuelve.
    	if(oeste[x][y] == false){
    		resolver(x-1, y);
    	}
    	
    	//Si encuentra el centro, termina.
    	if(encontrado){
    		return;
    	}
    	...
    	//Imprime el tiempo que ha tardado.
    	System.out.println("Tiempo total "+System.currentTimeMillis());
    }
